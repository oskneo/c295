...
int cmpstringp(const void *pa, const void *pb){
    return strcmp(* (float * ) pa, * (float * ) pb);
}
...
void main () {
    ...
    // qsort(arr1, .., .., ..);
    qsort_r(&arr1[0],24,sizeof(float),cmpstringp);
    ...
    // qsort(arr2, .., .., ..);
    qsort_r(&arr2[0],50,sizeof(float),cmpstringp);
